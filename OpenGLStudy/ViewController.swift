...
class ViewController: UIViewController {
    ...
    func drawMap(json: JSON) {
        ...
        let width = json[0]["Info"]["Width"].floatValue
        let height = json[0]["Info"]["Height"].floatValue
        mapView.bounds = CGRect(x: 0, y: 0, width: CGFloat(width), height: CGFloat(height))
        
        //draw point
        print("\(json[0]["Data"].arrayValue)")
        
        let length = json[0]["Info"]["Width"].intValue

        let oldArr = json[0]["Data"].arrayValue

        
        var newArr = Array(repeating:Array(repeating:Int(), count:length), count:length)
